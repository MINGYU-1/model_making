...
results = {
    "random_state": [],
    "R2_MSE": [],      # bce_binary * mse (최종)
    "R2_SOFT_LABEL": [],     # bce_prob * mse (sigmoid 가중)
    "R2_HARD_LABEL": []          # mse만 (x_hat_fin)
}
for i in np.random.randint(1,100,size = 20):
    ...
    results["R2_SOFT_LABEL"].append(float(r2_bce_mse_sig))
    results["R2_HARD_LABEL"].append(float(r2_bce_mse))

save_path = "./results_case1.json"
