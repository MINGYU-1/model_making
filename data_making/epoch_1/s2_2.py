...
from model1.m26odel_mse import Modelmse
from model1.m26odel_bce import Modelbce
from loss1.l26_mse_1 import loss_mse
from loss1.l26_bce_1 import loss_bce
import joblib
import torch
import numpy as np
import pandas as pd
...
for i in np.random.randint(1,100,size = 20):
    ...
    x_train,x_test = [torch.tensor(x, dtype = torch.float32) for x in [x_train,x_val,x_test]]
    c_train,c_test = [torch.tensor(c, dtype = torch.float32) for c in [c_train,c_val,c_test]]
    ...
    model = Modelbce(x_dim,c_dim, z_dim=8,z2_dim = 8).to(device)
    ...
    for epoch in range(1,epochs+1):
        ...
        for x,x2, c in train_loader:
            x,x2,c = x.to(device),c.to(device)
            optimizer.zero_grad()
            bce_logit, mu, logvar = model(x,c)
            loss_dict = loss_mse(bce_logit,x, mu)
        ...
        with torch.no_grad():
            for v_x, v_c in val_loader:
                ...
                loss_dict = loss_bce(v_bce_logit, v_x, v_mu,v_logvar)
...
for i in np.random.randint(1,100,size = 20):
    ...
    r2_bce_mse_sig = r2_score(x_true.flatten(),final_x_sig.flatten())
    ##각각의 r2저장
    results["R2_Wihtout_React"].append(float(r2_bce_mse_sig))
